...
function calc() {
	const part1 = Object.keys(launchRobot(input, false)).length;
	
	const map = launchRobot(input, true);
	drawMap(map);
...
function launchRobot(text, part2) {
	const map = {};
	const moves = [[0, -1], [1, 0], [0, 1], [-1, 0]];
	const coords = [0, 0];
	let heading = 0;

	const robot = new IntCode(text);
	...
	while (robot.running) {
		robot.input.push(map[coords.join(":")] || 0);
		
		robot.run();
		
		map[coords.join(":")] = robot.output.shift();
		
		heading += (robot.output.shift() == 1) ? 1 : -1;
